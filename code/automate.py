...
# hierarchy = d.dump(pretty=True)

# parser = etree.XMLParser(ns_clean=True, recover=True, encoding='utf-8')
# root = etree.fromstring(hierarchy.encode("utf-8"), parser=parser)
# print(etree.tostring(root))

# btn = d(clickable="true")
# btn = d(index="0",  text="11", resourceId="testcoverage.com.testcoverageapp:id/id")
...
class AndroidEnv:
    ...
    def reset(self):
        ...
        # gui_objects = self.device(clickable="true")#.child(clickable="true")
        # print(self.device.dump(compressed=False))
        # self.device.dump(compressed=True)
        # gui_objects = self.device(packageNameMatches="systemui")
        # print(len(gui_objects))
        # for o in gui_objects:
            # print(o.info)
        # actions = [Action(obj) for obj in gui_objects]
        return self._get_screen(), self._get_actions()

    def step(self, action):
        action.execute()
        # self.device.click(action[0], action[1])
        obs = self._get_screen()
        complete = self._image_to_torch(misc.imread("complete.png"))
        done = torch.equal(obs, complete)
        # done = False
        gui_objects = self.device(clickable="true")
        # actions = [Action(obj) for obj in gui_objects]
    ...
    def _get_actions(self):
        actions = []
        for gui_obj in self.device().child():
...
# for i_episode in count(1):
    ...
    # # env.reset()
    # last_screen = get_screen(d)
    # current_screen = get_screen(d)
    # state = current_screen - last_screen
    # for t in count():
        # # Select and perform an action
        # action = select_action(state)
        # # _, reward, done, _ = env.step(action[0,0])
        # action = d(clickable="true")[0]
        # action.click()
        # reward = 1
        # done = True
